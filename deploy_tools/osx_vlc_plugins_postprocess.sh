...
for DYLIB in $1/*.dylib;
do
  echo "[$SCRIPT] # Processing: '$DYLIB'"
  LIBVLCCORE=$(otool -L "$DYLIB" | grep '\(libvlccore\..[0-9]*\.dylib\)' -o)
  LPLIBVLCCORE=$(otool -L "$DYLIB" | grep '\(/opt/.*lib/libvlccore\..[0-9]*\.dylib\)' -o)
  if otool -L "$DYLIB" | grep '\(/opt/.*lib/libvlccore\..[0-9]*\.dylib\)' -o; then
    change_install_name $LPLIBVLCCORE @executable_path/../Frameworks/$LIBVLCCORE "$DYLIB"
  fi
done
