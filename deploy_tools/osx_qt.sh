...
QTCOREID=$(otool -L ${APP_NAME_FULL} | grep '\(/..*/QtCore.framework/Versions/5/QtCore\)' -o)
QTCORE=$(otool -L "$DYLIB" | grep '\(.*/QtCore.framework/Versions/5/QtCore\)' -o)

QTGUIID=$(otool -L ${APP_NAME_FULL} | grep '\(/..*/QtGui.framework/Versions/5/QtGui\)' -o)
QTGUI=$(otool -L "$DYLIB" | grep '\(.*/QtGui.framework/Versions/5/QtGui\)' -o)

QTNETWORKID=$(otool -L ${APP_NAME_FULL} | grep '\(/..*/QtNetwork.framework/Versions/5/QtNetwork\)' -o)
QTNETWORK=$(otool -L "$DYLIB" | grep '\(.*/QtNetwork.framework/Versions/5/QtNetwork\)' -o)

QTQUICKID=$(otool -L ${APP_NAME_FULL} | grep '\(/..*/QtQuick.framework/Versions/5/QtQuick\)' -o)
QTQUICK=$(otool -L "$DYLIB" | grep '\(.*/QtQuick.framework/Versions/5/QtQuick\)' -o)

QTQMLID=$(otool -L ${APP_NAME_FULL} | grep '\(/..*/QtQML.framework/Versions/5/QtQml\)' -o)
QTQML=$(otool -L "$DYLIB" | grep '\(.*/QtQml.framework/Versions/5/QtQml\)' -o)

QTWIDGETSID=$(otool -L ${APP_NAME_FULL} | grep '\(/..*/QtWidgets.framework/Versions/5/QtWidgets\)' -o)
QTWIDGETS=$(otool -L "$DYLIB" | grep '\(.*/QtWidgets.framework/Versions/5/QtWidgets\)' -o)

if [ -n "$QTCORE" ]; then
  change_install_name $QTCORE $QTCOREID "$DYLIB"
fi

if [ -n "$QTGUI" ]; then
  change_install_name $QTGUI $QTGUIID "$DYLIB"
fi

if [ -n "$QTNETWORK" ]; then
  change_install_name $QTNETWORK $QTNETWORKID "$DYLIB"
fi

if [ -n "$QTQUICK" ]; then
  change_install_name $QTQUICK $QTQUICKID "$DYLIB"
fi

if [ -n "$QTQML" ]; then
  change_install_name $QTQML $QTQMLID "$DYLIB"
fi

if [ -n "$QTWIDGETS" ]; then
  change_install_name $QTWIDGETS $QTWIDGETSID "$DYLIB"
fi
